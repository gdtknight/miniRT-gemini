...
버전 4.0.0에 대한 근거:
- 메이저 버전 업데이트: 비협상 원칙인 이중 언어 명세의 디렉토리 구조가 변경되었습니다. 이는 해당 경로에 의존하는 모든 자동화 또는 도구에 대한 파괴적인 변경으로 간주됩니다.
- 수정된 원칙 VI (이중 언어 명세 관리): 한국어 명세 문서의 경로가 언어별 프로젝트 아티팩트를 더 잘 그룹화하기 위해 `docs/specs/`에서 `specs/kr/`로 변경되었습니다.

정의된 원칙 (총 12개):
1. 프로젝트 구조 표준 (협상 불가)
2. 코드 품질 자동화 (협상 불가)
3. 문서 및 Wiki 동기화 (협상 불가)
4. 워크플로우 시스템 (협상 불가)
5. 도구 및 환경 표준 (협상 불가)
6. 이중 언어 명세 관리 (협상 불가)
7. 42 School 함수 제약 조건 (선택 사항)
8. 보안 표준 (협상 불가)
9. 브랜칭 전략 (협상 불가)
10. 커밋 메시지 규칙 (협상 불가)
11. 테스트 전략 및 커버리지 (협상 불가)
12. 폐기 정책 (협상 불가)

업데이트가 필요한 템플릿:
✅ `docs/specs/` 경로를 하드코딩한 템플릿을 찾지 못했습니다.

후속 조치:
- 한국어 명세 파일을 `docs/specs/`에서 `specs/kr/`로 수동으로 이동해야 합니다.
-->
...
**문서 계층 구조**:
- 모든 문서는 `docs/` 디렉토리에 중앙 집중화되어야 하며, `spec`, `wiki`, `archive` 등을 위한 하위 디렉토리를 가집니다.
...
GitHub Wiki는 사용자가 항상 최신 정보에 접근할 수 있도록 `docs/` 디렉토리의 소스 문서와 자동으로 동기화되어야 합니다.

**자동화 요구사항**:
- 버전 태그 생성은 자동 GitHub Wiki 업데이트를 트리거해야 합니다.
- `docs/` 디렉토리는 단일 진실 공급원입니다.
...
**버전**: 4.0.0 | **비준일**: 2025-12-27 | **최종 수정일**: 2025-12-27
