...
버전 5.1.0에 대한 근거:
- 마이너 버전 업데이트: 이슈 기반 개발 워크플로우와 커밋 메시지 규칙에 대한 새로운 원칙이 추가되었습니다. 이는 기존 워크플로우에 새로운 요구사항을 추가하는 하위 호환되는 변경입니다.

수정된 원칙:
- 원칙 X (커밋 메시지 규칙): 커밋 메시지에 관련 GitHub 이슈 번호를 포함하도록 규칙이 수정되었습니다.
- 추가된 원칙 XIII (이슈 기반 개발 워크플로우): 모든 개발 작업이 GitHub 이슈에서 시작되어야 하며, 일관성을 위해 이슈 및 PR 템플릿을 사용해야 한다는 새로운 원칙이 추가되었습니다.

업데이트가 필요한 템플릿:
⚠ `.github/ISSUE_TEMPLATE/` 및 `.github/PULL_REQUEST_TEMPLATE.md` 경로에 템플릿 파일들을 생성해야 합니다.

후속 조치:
- GitHub 이슈 및 PR 템플릿을 생성하고 `.github/` 디렉토리에 추가해야 합니다.
-->
...
모든 커밋 메시지는 명시적이고 쉽게 읽을 수 있는 프로젝트 히스토리를 생성하기 위해 **Conventional Commits** 사양을 따라야 합니다. 모든 커밋은 관련 GitHub 이슈 번호를 포함해야 합니다.
...
- **issue-number**: 관련된 GitHub 이슈 번호.

**자동화**:
- CI 검사는 PR 제목이나 커밋 메시지가 이 형식을 준수하는지 확인해야 합니다. 이는 변경 로그의 자동 생성 및 버전 관리를 단순화합니다.

**근거**: 표준화된 커밋 히스토리는 기계가 읽을 수 있고 개발자 간의 의사소통을 개선하여 변경 사항을 이해하고 릴리스 노트를 자동화하기 쉽게 만듭니다. 이슈 번호 포함은 코드 변경과 요구사항 간의 직접적인 추적을 가능하게 합니다.
...
### XIII. 이슈 기반 개발 워크플로우 (협상 불가)

모든 개발 작업은 추적 가능성과 명확성을 보장하기 위해 GitHub 이슈를 중심으로 진행되어야 합니다.

**작업 흐름**:
1.  **이슈 생성**: 모든 기능 개발, 버그 수정, 리팩토링은 GitHub 이슈로 먼저 정의되어야 합니다.
2.  **라벨 및 브랜치 할당**: 작업 시작 전, 이슈에는 적절한 `label`(예: `feature`, `bug`)이 할당되어야 하며, 해당 이슈 작업을 위한 전용 개발 브랜치가 생성 및 할당되어야 합니다.
3.  **작업 진행**: 할당된 브랜치에서 개발을 진행합니다.

**템플릿 강제**:
- GitHub 이슈와 풀 리퀘스트(PR)는 일관성을 위해 사전에 정의된 템플릿에 따라 작성되어야 합니다.
- 이 템플릿들은 `.github/` 디렉토리에 위치해야 합니다.

**근거**: 이슈 기반 워크플로우는 모든 변경 사항의 목적과 논의 과정을 중앙에서 관리할 수 있게 하여 프로젝트의 투명성을 높입니다. 템플릿 사용은 품질과 일관성을 보장합니다.
...
**버전**: 5.1.0 | **비준일**: 2025-12-27 | **최종 수정일**: 2025-12-27
